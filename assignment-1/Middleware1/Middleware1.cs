...
public class MiddlewareForm : Form
{
    ...
    const int MIDDLEWARE_ID = 1;
    const int MY_PORT       = 8082;
    // ====================================

    const int NUM_MIDDLEWARE = 5;
    const int BASE_PORT      = 8081; // middleware N listens on BASE_PORT + N
    ...
    // Step 2: received a multicast — propose a timestamp and buffer it
    void HandleMulticast(string msgId, string text)
    {
        ...
        // Send PROPOSE back to the original sender
    ...
    // Step 3: collected a proposal — when all arrive, agree on max and send FINAL
    ...
    // Step 4 & 5: received final timestamp — update, re-sort, deliver from head
    void HandleFinal(string msgId, int value, int senderID)
    {
        ...
        lock (stateLock)
        {
            ...
            // Deliver consecutive final messages from the head
